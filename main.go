...
func main() {
    ...
    s3_conf, err_s3 := config.LoadDefaultConfig(context.TODO(), config.WithRegion("us-east-2") );
    if err_s3 != nil {
        log.Fatal("Failed to load config for s3: %v\n", err_s3);
    }

    s3_client := s3.NewFromConfig(s3_conf);
    log.Printf("%v\n", s3_client);
